#you can replace "CPU_1", "CPU_2" etc with whatever text you want, it's purely aesthetic
#warning: do not use spaces or this will break

team join player CPU_1
team join player CPU_2
team join player CPU_3
team join player CPU_4
team join player CPU_5
team join player CPU_6
team join player CPU_7
team join player CPU_8
team join player CPU_9